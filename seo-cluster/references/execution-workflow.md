...
priority ordering, context injection for whatever writes the content, backward link
updates, resume capability, and post-execution quality scoring.
...
When handing each post to whatever writes the content (a content-generation
skill, a downstream LLM call, or a brief for a human writer), pass a structured
context block:
