...
- Use web search, or a dedicated SERP-data API if you have one
...
|--------------|-------------|--------|
| 7-10 | **Same post** | Merge keywords into one target page. Use the higher-volume keyword as primary. |
| 4-6 | **Same cluster** | Place in the same spoke cluster. May be separate posts or the same post depending on volume difference. |
| 2-3 | **Interlink** | Place in adjacent clusters. Create cross-cluster internal links. |
| 0-1 | **Separate** | Different clusters entirely, or exclude from the current pillar topic. |
...
4. When in doubt, keep in the same cluster with separate posts (err toward cohesion)
...
1. **A dedicated SERP-data API** (e.g. DataForSEO), if you have access to one: most
   reliable, consistent SERP data. Check your own cost/rate budget before running a
   large batch of lookups.
2. **Plain web search** (fallback): adequate for clustering, but results may vary by
   session. Run multiple searches for the same keyword and use the most common
   result set when a score lands near a threshold boundary.
