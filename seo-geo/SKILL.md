...
Frame every GEO finding as **SEO fundamentals applied to AI-search
surfaces**, not as a separate optimization discipline. When a community
recommendation contradicts Google's primary source, defer to Google and
note the contradiction explicitly in the output.
...
Read `references/llmstxt-evidence.md` for the primary-source evidence (Mueller, Illyes, SE Ranking 300k-domain study, OtterlyAI server-log audit) on why `/llms.txt` is not currently a citation lever for major AI search systems. Report its presence when auditing a site, but assign it no citation-ranking weight.
...
Generate a `GEO-ANALYSIS.md` report with:
...
---
Adapted from published SEO/GEO research (Google Search Central, Ahrefs, SparkToro, SE Ranking, OtterlyAI) and the open-source `claude-seo` project (MIT license) — see `LICENSE.txt`.
